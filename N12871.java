...
public class N12871 {
    public static void main(String[] args) throws IOException {
        ...
        int gcd = gcd(Math.max(sLength, tLength), Math.min(sLength, tLength));
        int lcm = sLength * tLength / gcd;

        int sMul = lcm / sLength;
        int tMul = lcm / tLength;

        String sResult = "", tResult = "";
        for(int i = 0; i < sMul; i++) {
            sResult += s;
        }
        for(int i = 0; i < tMul; i++) {
            tResult += t;
        }

        if(sResult.equals(tResult)) System.out.println(1);
