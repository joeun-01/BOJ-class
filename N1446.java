...
public class N1446 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int D = Integer.parseInt(stoi.nextToken());

        // 길을 쭉 가다가 지름길이 있으면 타는데
        // 지름길을 무작정 타면 안되고
        // 지름길을 탔을 때와 안탔을 때로 계속 분기해야 하는데

        for(int i = 0; i < N; i++) {
            stoi = new StringTokenizer(read.readLine());

            int start = Integer.parseInt(stoi.nextToken());
            int end = Integer.parseInt(stoi.nextToken());
            int dist = Integer.parseInt(stoi.nextToken());
