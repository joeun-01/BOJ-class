...
public class N17219 {
    public static void main(String[] args) throws IOException {
        ...
        int find = Integer.parseInt(input.nextToken());

        HashMap<String, String> password = new HashMap<>();

        for(int i = 0; i < page; i++) {
            StringTokenizer homePage = new StringTokenizer(read.readLine());

            password.put(homePage.nextToken(), homePage.nextToken());
        }

        for(int i = 0; i < find; i++) {
            build.append(password.get(read.readLine()) + "\n");
        }
