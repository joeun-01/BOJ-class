...
따라서, 사전에서 단어 N개를 찾아 화면에 띄운 다음, 입력해야 하는 단어 순서를 알려주는 방식을 사용하기로 했다.

단어의 목록과 각 참가자의 순서가 주어졌을 때, 모든 사람의 비밀번호를 구하는 프로그램을 작성하시오.

입력은 여러 개의 테스트 케이스로 이루어져 있고, 첫째 줄에 테스트 케이스의 개수가 주어진다.

각 테스트 케이스의 첫째 줄에는 단어의 개수 1 ≤ m ≤ 50이 주어진다. 
다음 m개 줄에는 단어가 한 줄에 하나씩 주어진다. 
각 단어의 길이는 최대 5글자이고, 공백을 포함하지 않는다. 
다음 줄에는 참가자의 수 1 ≤ n ≤ 200이 주어지며, 다음 n개 줄에는 각 참가자의 비밀번호 정보가 주어진다. 

비밀번호 정보는 정수 1 ≤ k ≤ 10 로 시작하며, 비밀번호를 이루고 있는 단어의 개수이다. 
다음 k개 숫자는 단어의 인덱스를 나타내며, 순서대로 해당하는 단어를 연결한 문자열이 비밀번호이다.
...
public class N7513 {
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 1; i <= T; i++) {
            ...
            int M = Integer.parseInt(read.readLine());  // 단어의 개수

            for(int j = 0; j < M; j++) {
                array.add(read.readLine());
            }

            int N = Integer.parseInt(read.readLine());  // 참가자 개수
            for(int j = 0; j < N; j++) {
                StringTokenizer stoi = new StringTokenizer(read.readLine());
                int K = Integer.parseInt(stoi.nextToken());

                for(int k = 0; k < K; k++) {
                    build.append(array.get(Integer.parseInt(stoi.nextToken())));
                }
