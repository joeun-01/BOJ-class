...
따라서, 현우는 옥포 해전이 가장 먼저 일어난 해전인지 마지막에 일어난 해전인지 생각해내지 못했고, 다음과 같이 결국 제일 마지막에 적고말았다.
...
public class N3077 {
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 1; i <= N; i++) {    
            map.put(stoi.nextToken(), i);
        }

        String[] answer = new String[N];
        stoi = new StringTokenizer(read.readLine());
        for(int i = 0; i < N; i++) {
            answer[i] = stoi.nextToken();
        }

        int score = 0;
        for(int i = 0; i < N; i++) {
            for(int j = i + 1; j < N; j++) {
                int first = map.get(answer[i]);
                int second = map.get(answer[j]);
