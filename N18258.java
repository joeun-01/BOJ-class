...
public class N18258 {
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < N; i++) {
            ...
            String oper = stoi.nextToken();

            if(oper.equals("push")) {
                int num = Integer.parseInt(stoi.nextToken());
                queue.add(num);
            } else if(oper.equals("pop")) {
                if(queue.isEmpty()) build.append(-1).append("\n");
                else build.append(queue.poll()).append("\n");
            } else if(oper.equals("size")) {
                build.append(queue.size()).append("\n");
            } else if(oper.equals("empty")) {
                if(queue.isEmpty()) build.append(1).append("\n");
                else build.append(0).append("\n");
            } else if(oper.equals("front")) {
                if(queue.isEmpty()) build.append(-1).append("\n");
                else build.append(queue.getFirst()).append("\n");
            } else if(oper.equals("back")) {
                if(queue.isEmpty()) build.append(-1).append("\n");
                else build.append(queue.getLast()).append("\n");
            }
        }
