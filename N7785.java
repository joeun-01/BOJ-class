...
public class N7785 {
    public static void main(String[] args) throws IOException {
        ...
        TreeSet<String> enterance = new TreeSet<>(Collections.reverseOrder());
        for(int i = 0; i < N; i++) {
            StringTokenizer stoi = new StringTokenizer(read.readLine());
            String name = stoi.nextToken();
            String enter = stoi.nextToken();

            if(enter.equals("enter")) enterance.add(name);
            else if(enter.equals("leave")) enterance.remove(name);
        }   

        Iterator<String> iter = enterance.iterator();
        while(iter.hasNext()) {
