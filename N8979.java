...
public class N8979 {
    public static class Medal {
        ...
        public Medal(int country, int gold, int silver, int copper) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < N; i++) {
            ...
            int gold = Integer.parseInt(stoi.nextToken());
            int silver = Integer.parseInt(stoi.nextToken());
            int copper = Integer.parseInt(stoi.nextToken());

            medal[i] = new Medal(country, gold, silver, copper);
        }

        Arrays.sort(medal, new Comparator<Medal>() {
            @Override
        ...
        for(int i = 1; i < N; i++) {
            if(medal[i].gold == medal[i - 1].gold && medal[i].silver == medal[i - 1].silver && medal[i].copper == medal[i - 1].copper) {
                medal[i].rank = medal[i - 1].rank;
            } else {
                medal[i].rank = medal[i - 1].rank + 1;
            }
        }

        for(int i = 0; i < N; i++) {
            if(medal[i].country == K) {
                System.out.println(medal[i].rank);
                break;
            }
        }
    }
